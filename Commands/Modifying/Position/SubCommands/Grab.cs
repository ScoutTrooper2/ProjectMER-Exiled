...
public class Grab : ICommand
{
	...
	public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
	{
		...
		if (GrabbingPlayers.ContainsKey(player))
		{
			...
			mapEditorObject.Base.Position = room.Type == Exiled.API.Enums.RoomType.Surface ? mapEditorObject.transform.position : mapEditorObject.Room.Transform.InverseTransformPoint(mapEditorObject.transform.position);
	...
	private IEnumerator<float> GrabbingCoroutine(Player player, MapEditorObject mapEditorObject)
	{
		...
		while (true)
		{
            ...
            if (mapEditorObject.Base is SerializableDoor _ || mapEditorObject.Base is SerializableLocker _)
			{
